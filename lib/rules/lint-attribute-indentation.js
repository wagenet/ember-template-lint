...
module.exports = class AttributeSpacing extends Rule {
  ...
  validateParams(node) {
    ...
    */
    ...
    if (node.type === 'ElementNode') {
      expectedLineStart = this.iterateParams(
        node.attributes,
        'htmlAttribute',
        expectedLineStart,
        expectedColumnStart,
  ...
  validateBlockForm(node) {
    if (
      (node.type === 'ElementNode' && node.attributes.length) ||
      node.params.length ||
    ...
    if (node.type === 'ElementNode') {
      ...
      const expectedStartLine =
        lastChild.type === 'BlockStatement' ? lastChild.loc.end.line + 1 : lastChild.loc.end.line;
